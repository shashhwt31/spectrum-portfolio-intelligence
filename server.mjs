...
async function load(){if(!existsSync(storePath))return {holdings:demoHoldings,audit:[{at:now(),event:'demo_seed_loaded',source:'system'}]};return JSON.parse(await readFile(storePath,'utf8'))}
...
const server=createServer(async(req,res)=>{try{
 ...
 if(url.pathname==='/api/portfolio'&&req.method==='GET'){const data=await load();return send(res,200,{holdings:data.holdings,analysis:analyze(data.holdings),audit:data.audit.slice(-10)})}
